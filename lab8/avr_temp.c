...
//		fprintf(fp, "ADC VALUE IS: %d\n", ADC_output);
		...
		vin = ((float)ADC_output * 1.1) / 1024.0;

		temp = (vin - 0.290) / 0.001;		
...
void init_ADC(void) {
	...
	// Set ADMUX register to use 1.1V internal reference voltage for ADC source
	ADMUX = 0;
	ADMUX |= (1 << REFS0) | (1 << REFS1);
	
	// Sets the input to be the temp sensor
	ADMUX |= (1 << MUX3);
